...
def assignPoints(X, rdd_data, Mcurr, Dis):
	N, d = np.shape(rdd_data)
	assigns = np.ones(N) * -1
	#for i in range(y.shape[0]):
	minDist = np.ones(N) * 10000 #np.inf
	best = -1
	...
	for i in range(len(Mcurr)):
		...
		for j in range(len(assigns)):
			if Mcurr[i] == assigns[j]:
				clust.append(j)
		C = X.filter(lambda x: any(x[0] == clust[k] for k in range(len(clust)))).cache()
		#print(C.count()) # number of points in cluster
		#c_size = len(clust)  # number of points in cluster
		Cm = C.mean()#reduce(lambda x,y: np.add(x,y)) / c_size # cluster centroid
		#print(Cm) # print centroid
		Ysum = 0.0

		for d in Dis[i]:
			gs = C.map(lambda x: np.abs(x[d] - Cm[d]))#.cache()
			g = gs.mean()#reduce(lambda x,y: np.add(x,y))
			Ysum += g #/ c_size # avg dist to centroid along dim d:
		wi = Ysum / len(Dis[i])
		#print("wi is :",wi) # weighted average of cluster
		upperSum += len(clust) * wi
...
def proclus(X, k, l, minDeviation = 0.05, A = 10, B = 3, niters = 2, seed = 1234):
	...
	N = np.shape(np.array(rdd_data))[0]
	...
	while True:
		...
		Dis = findDimensions(X, rdd_data, k, l, L, Mcurr) # find the dimensions which are good in ascending order
		print("dimensions are:",Dis)
		# form the clusters:
		assigns = assignPoints(X, rdd_data, Mcurr, Dis) # assign points based on average dimension "l"
	...
	Dis = findDimensions(X, rdd_data, k, l, L, Mcurr) #repeat again: find dimension.
	assigns = assignPoints(X, rdd_data, Mcurr, Dis)

	print("m curr is ",Mcurr)# handle outliers: mark them as -1
	...
	return (Mcurr, Dis, assigns) #Mcurr is medoid centers,Dis are dimensions used and assigns are cluster assigns
...
def findDimensions(X, rdd_data, k, l, L, Mcurr):
	N, d = np.shape(rdd_data)
	...
	for i in range(len(Mcurr)):
		...
		m = rdd_data[mi]
		Davg = X.filter(lambda x: any(x[0] == L[i][j] for j in range(len(L[i]))))# Filter all data points in the locality sphere
		#print("d avg count",Davg.count())
		Xi = Davg.map(lambda x: np.abs(x - m)).reduce(lambda x,y: np.add(x,y))## Xij here is an array, containing the avg dists in each dimension
		#print("xi rdd is",Xi.first())
		#Xij = Xi.mean()
		#print(Xij)
		Xij = Xi / len(L[i]) # Xij is the average distance from the points in L_i to m_i
		print("XIJ IS :",type(Xij))
...
def dist_near(X,data,val,mi):
    D = X.map(lambda x: np.linalg.norm(data[mi] - x)).collect()  # map and reduce
    #print("nearby dist:",D)
    mind = []
...
loc = "/usr/lib/spark/examples/src/main/python/Major_proj/datasets/Archive/3MillionG12cluster.csv"   # give dataset location here

#FOR SPARK RDD
dat = sc.textFile(loc,100) # get dataset into rdd and number of partitions
...
if R: # run proclus
	...
	l = 4    #get no of dimensions 
	M, D, A = proclus(data_rdd, k, l, seed = rseed) #running proclus code
